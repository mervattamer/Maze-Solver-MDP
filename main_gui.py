...
# Define Button class
class Button:
    def __init__(self, x, y, width, height, color, text, callback, screen,font ):
    ...
    def handle_event(self, event,n, test_maze, test_mdp):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.rect.collidepoint(event.pos):
                pygame.quit()

                self.callback(n, test_maze, test_mdp)

# Define button callbacks
def button1_callback(n, test_maze, test_mdp):
    # print(n)
...
def button2_callback(n, test_maze, test_mdp):
...
def mainn(test_maze, test_mdp, n):
    test_maze = test_maze
    test_mdp = test_mdp
    ...
    pygame.display.set_caption("Maze Solver")
    font = pygame.font.Font(None, 36)
    button1 = Button(50, 100, 100, 50, (0, 255, 0), "Policy", button1_callback, screen=screen, font=font)
    button2 = Button(250, 100, 100, 50, (0, 0, 255), "Value", button2_callback, screen=screen, font=font)
# Main game loop
    while True:
        for event in pygame.event.get():
            ...
            button1.handle_event(event,n, test_maze, test_mdp)
            button2.handle_event(event,n, test_maze, test_mdp)
